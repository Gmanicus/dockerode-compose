...
function standardizeVolumeStr(volumeStr, cwd) {
  ...
  return path.resolve(cwd, localPath) + ':' + parts.slice(1);
}
...
module.exports = {
  ...
  /** Replaces all service environment variable references with the corresponding env value if found.
   * Otherwise, the reference will be replaced with null. Returns the env variable array.
   * 
   * @param {*} service 
   * @param {*} options 
   */
  configureEnvironment: function (service, options) {
    const envReferenceRegex = /\$\{(.*?)\}/;
    const envValueRegex = /.*?=(.*)/;
    let vars = [];

    function getMappedProp(prop) {
      const propEnv = envReferenceRegex.exec(prop)?.slice(1);
      if (!propEnv) return prop;
      for (const envVar of propEnv) {
        // For every env reference '${xyz}' in the prop
        const foundVar = vars.find((env) => env.includes(`${envVar}=`));
        if (foundVar) {
          // If we find it in the enviroment, replace the prop reference with the env value
          const envValue = (foundVar) ? envValueRegex.exec(foundVar)[1] : null;
          prop = prop.replace('${' + envVar + '}', envValue);
        } else if (options?.envOverrides[envVar]) {
          // Otherwise, check to see if it is included in the overrides
          prop = prop.replace('${' + envVar + '}', options.envOverrides[envVar]);
        }
      }
      return prop;
    };
    
    function mapObjPropsToEnv(obj) {
      for (const prop in obj) {
        if (Array.isArray(obj[prop])) {
          obj[prop] = obj[prop].map((aprop) => {
            if (typeof aprop == 'string') {
              return getMappedProp(aprop);
            } else {
              return aprop;
            }
          })
        } else if (typeof obj[prop] == 'object') {
          mapObjPropsToEnv(obj[prop], vars);
        } else if (typeof obj[prop] == 'string') {
          obj[prop] = getMappedProp(obj[prop], vars);
        }
      }
    };

    vars = this.buildEnvVars(service);

    // If a variable matches an environment variable passed via options, overwrite it
    for (const [index, env] of vars.entries()) {
      const envVar = env.split('=')[0];
      if (Object.keys(options?.envOverrides || {}).includes(envVar)) {
        vars[index] = `${envVar}=${options.envOverrides[envVar]}`;
      } else {
      }
    }

    // Map environment variable references in the service speficication to values provided by the environment
    mapObjPropsToEnv(service);
    return vars;
  },
  ...
  buildDockerImage: async function (
    ...
    if (dockerfile !== null) {
      ...
      streami = await docker.buildImage(
        {
          context: buildPath,
          src: [dockerfile],
        },
      ...
      streami = await docker.buildImage(tarStream, obj);
    }
    ...
    if (options.streamCallback) options.streamCallback(streami);
